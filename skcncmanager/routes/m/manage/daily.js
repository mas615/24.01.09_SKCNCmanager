...
router.get('/', function(req, res, next) {
    ...
    sql = `SELECT id, part, ranking, project, type, DATE_FORMAT(date1, '%y-%m-%d') AS date1, DATE_FORMAT(date2, '%y-%m-%d') AS date2, DATE_FORMAT(date3, '%y-%m-%d') AS date3, DATE_FORMAT(date4, '%y-%m-%d') AS date4, tester, status, memo1, memo2 FROM daily_table order by 2`;
    data =[];
